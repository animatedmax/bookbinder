describe('external links', function() {
  it('opens an external link in a new window when clicked', function() {
    ...
    expect(Bookwatch.needsNewWindow(event, 'http://other.example.com/foo', 'thing.example.com')).toBe(true);
  });

  it('opens an SSL external link in a new window when clicked', function() {
    var event = clickEvent({ mouseButton: 0 });

    expect(Bookwatch.needsNewWindow(event, 'https://other.example.com/foo', 'thing.example.com')).toBe(true);
  });

  it('does not open a new window for a link to the current domain', function() {
    var event = clickEvent({ mouseButton: 0 });

    expect(Bookwatch.needsNewWindow(event, 'https://thing.example.com/foo', 'thing.example.com')).toBe(false);
  });
  ...
  it('does not open a new window for an external link when the user uses a non-primary mouse button', function() {
    ...
    expect(Bookwatch.needsNewWindow(event, 'https://other.example.com/foo', 'thing.example.com')).toBe(false);
  });
