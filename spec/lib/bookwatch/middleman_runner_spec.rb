...
module Bookwatch
  describe MiddlemanRunner do
    ...
    let(:sheller) { instance_double('Bookwatch::Sheller') }
    ...
    it "stores bookwatch config for later consumption by our extension" do
      allow(sheller).to receive(:run_command)
      run_middleman
      expect(fs.received_to).to eq('bookwatch_config.yml')
