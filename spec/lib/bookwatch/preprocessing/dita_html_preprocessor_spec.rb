...
require_relative '../../../../lib/bookwatch/preprocessing/dita_html_preprocessor'
    ...
    describe DitaHTMLPreprocessor do
      ...
      context "for dita sections with a ditamap" do
        it "generates a subnav for each section with ditamap" do
          ...
          subnav_generator = instance_double(Bookwatch::Subnav::SubnavGenerator)
          subnav_generator_factory = instance_double(Bookwatch::Subnav::SubnavGeneratorFactory)

          expect(subnav_generator_factory).to receive(:produce).with(instance_of(Bookwatch::Subnav::NavigationEntriesFromHtmlToc)) { subnav_generator }
        ...
        it "runs the dita command and raises an exception on failure" do
          command_creator = instance_double(Bookwatch::DitaCommandCreator)
          sheller = instance_double(Bookwatch::Sheller)
      ...
      it "copies html and images for each section and its dependent sections" do
        ...
        dita_formatter = instance_double(Bookwatch::DitaHtmlForMiddlemanFormatter)
        stub_const('Bookwatch::Preprocessing::DitaHTMLPreprocessor::ACCEPTED_IMAGE_FORMATS', ['img-format'])
