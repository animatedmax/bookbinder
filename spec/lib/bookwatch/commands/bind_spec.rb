require_relative '../../../../lib/bookwatch/commands/bind'
require_relative '../../../../lib/bookwatch/commands/components/bind/directory_preparer'
...
require_relative '../../../../lib/bookwatch/server_director'
...
module Bookwatch
  describe Commands::Bind do
    ...
    let(:null_preprocessor) { instance_double('Bookwatch::Preprocessing::LinkToSiteGenDir', preprocess: nil) }
    let(:null_middleman_runner) { instance_double('Bookwatch::MiddlemanRunner', run: success) }
    let(:null_logger) { double('deprecated logger').as_null_object }
    let(:null_directory_preparer) { instance_double('Bookwatch::Commands::Components::Bind::DirectoryPreparer', prepare_directories: nil) }
    let(:null_cloner) { double('cloner').as_null_object }
    let(:null_cloner_factory) { instance_double('Bookwatch::Ingest::ClonerFactory', produce: null_cloner) }
    ...
    it "copies a redirects file from the current directory to the final app directory, prior to site generation" do
      ...
      generator = instance_double('Bookwatch::MiddlemanRunner')
    ...
    it "returns a nonzero exit code when Middleman fails" do
      middleman_runner = instance_double('Bookwatch::MiddlemanRunner')
      fs = instance_double('Bookwatch::LocalFilesystemAccessor')
    ...
    it "writes required files to output directory and outputs success message" do
      fs = instance_double('Bookwatch::LocalFilesystemAccessor', file_exist?: false)
      ...
      Commands::Bind.new(
        ...
        config_decorator: double('decorator', generate: config),
        file_system_accessor: fs,
        middleman_runner: instance_double('Bookwatch::MiddlemanRunner', run: success),
        link_checker: null_link_checker,
        preprocessor: null_preprocessor,
