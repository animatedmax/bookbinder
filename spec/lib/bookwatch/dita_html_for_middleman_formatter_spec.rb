...
require_relative '../../../lib/bookwatch/html_document_manipulator'

module Bookwatch
  describe DitaHtmlForMiddlemanFormatter do
    let(:fs) { instance_double('Bookwatch::LocalFilesystemAccessor', find_files_with_ext: ['thing.html'], read: html_text) }
    let(:html_doc_manipulator) { instance_double('Bookwatch::HtmlDocumentManipulator') }
